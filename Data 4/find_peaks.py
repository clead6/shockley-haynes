...
for key in keys:
    ...
    peaks, properties = find_peaks(volt, distance=1500, width=100, height = -5)
    ...
    unc_fwhm = np.append(unc_fwhm, 0.01*2*np.sqrt(2*np.log(2))*(time[int(properties["right_ips"])] - time[int(properties["left_ips"])]))
...
x_data = np.linspace(0, 4e-14, 1000)
y_unc = (d*fwhm)**2*np.sqrt(2*d*(unc_d*fwhm)**2+2*fwhm*(unc_fwhm*d)**2)
...
x_data = np.linspace(1.2e-5,3.5e-5,1000)
...
#plt.errorbar(t_0, np.log(abs(n_0/d**2)*np.sqrt(t_0)), yerr=y_unc, ls="none")
plt.xlabel(r"$t_0 [s]$")
